...
def OnMidiMsg(event):
    if event.midiId == midi.MIDI_SYSEX:
        handle_sysex_message(event)

    event.handled = True

# Function to handle SysEx messages
def handle_sysex_message(event):
    data = event.sysex
    if len(data) < 9:
        return

    # Check if it is a channel fader message
    if data[0:5] == [0x00, 0x20, 0x32, 0x20, 0x01]:
        fader_number = data[5]
        fader_value = data[6]
        set_channel_fader(fader_number, fader_value)
    
    # Check if it is a master fader message
    elif data[0:8] == [0x00, 0x20, 0x32, 0x00, 0x20, 0x02, 0x40, 0x01]:
        # Combine the fader value bytes
        fader_value = (data[12] << 7) | data[13]
        set_master_fader(fader_value)
    
    # Check if it is a pan control message
    elif data[0:5] == [0x00, 0x20, 0x32, 0x20, 0x01]:
        pan_number = data[5]
        pan_value = data[6]
        set_channel_pan(pan_number, pan_value)

# Function to set channel fader volume
def set_channel_fader(fader_number, value):
    if 0 <= fader_number < 32:  # Assuming faders 0-31 correspond to channels 1-32
        channel = fader_number
        volume = value / 127.0
        mixer.setTrackVolume(channel + 1, volume)  # Adjust for FL Studio's 1-based indexing

# Function to set master fader volume
def set_master_fader(value):
    volume = value / 16383.0  # Master fader range is 0-16383
    mixer.setTrackVolume(mixer.trackNumber("Master"), volume)

# Function to set channel pan
def set_channel_pan(pan_number, value):
    if 0 <= pan_number < 32:  # Assuming pan knobs 0-31 correspond to channels 1-32
        channel = pan_number
        pan = (value - 64) / 64.0  # Convert 0-127 to -1.0 to 1.0 range
        mixer.setTrackPan(channel + 1, pan)  # Adjust for FL Studio's 1-based indexing

# Function called when the script initializes
def OnInit():
    print("Behringer DDX3216 script initialized")

# Function called when the script is terminated
def OnDeInit():
    print("Behringer DDX3216 script terminated")

# Constants for note events
...
class TDDX3216CU:
    def __init__(self):
        ...
        self.DDX3216CU_PageNameT = ('Panning (press to reset)', 
    ...
    def OnInit(self):
        ...
        for m in range(len(self.FreeCtrlT)):
				...
				if (event.pmeFlags & midi.PME_System != 0):
					...
					if event.data1 == 0x34: # display mode
						...
						if self.Page == DDX3216CUPage_Free:
							...
							n = event.data1 - 0x20
							if self.Page == DDX3216CUPage_Sends:
	...
	def SetPage(self, Value):
		...
		self.FirstTrack = int(self.Page == DDX3216CUPage_Free)
		...
		if (oldPage == DDX3216CUPage_Free) | (self.Page == DDX3216CUPage_Free):
			self.UpdateMeterMode()
		self.UpdateColT()
		self.UpdateLEDs()
		self.UpdateTextDisplay()
	...
	def UpdateMixer_Sel(self):
		...
		if self.Page !=  DDX3216CUPage_Free:
			...
			if self.Page in [DDX3216CUPage_Sends, DDX3216CUPage_FX]:
				self.UpdateColT()
	...
	def UpdateColT(self):
		...
		for m in range(0, len(self.ColT)):
			if self.Page == DDX3216CUPage_Free:
				# free controls
				if m == 8:
					self.ColT[m].TrackNum = DDX3216CU_nFreeTracks
				else:
					self.ColT[m].TrackNum = (f + m) % DDX3216CU_nFreeTracks
				...
				if m == 8:
					...
					if self.Page == DDX3216CUPage_Pan:
					...
					elif self.Page == DDX3216CUPage_Stereo:
					...
					elif self.Page == DDX3216CUPage_Sends:
					...
					elif self.Page == DDX3216CUPage_FX:
					...
					elif self.Page == DDX3216CUPage_EQ:
						if m < 3:
							# gain & freq
					...
					if self.Flip:
						...
						self.ColT[m].KnobName = s
						self.ColT[m].KnobMode = 2
						if not (self.Page in [DDX3216CUPage_Sends, DDX3216CUPage_FX, DDX3216CUPage_EQ]):
							self.ColT[m].KnobCenter = -1
							self.ColT[m].KnobResetValue = round(12800 * midi.FromMIDI_Max / 16000)
							self.ColT[m].KnobResetEventID = self.ColT[m].KnobEventID
	...
	def SetFirstTrack(self, Value):
		...
		if self.Page == DDX3216CUPage_Free:
			self.FirstTrackT[self.FirstTrack] = (Value + DDX3216CU_nFreeTracks) % DDX3216CU_nFreeTracks
	...
	def OnIdle(self):
		...
		# refresh meters
		if device.isAssigned():
			f = self.Page == DDX3216CUPage_Free
			for m in range(0,  len(self.ColT) - 1):
...
# Function to set master fader volume
def set_master_fader(value):
    volume = value / 127.0  # Adjust the division if the master fader uses a different range
    mixer.setTrackVolume(mixer.trackNumber("Master"), volume)  # Using "Master" track name for main mix
